        ...
        while self.cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    ring_finger_distal_angle = 0
                    # self.get_logger().info(f"无名指近端关节角: {ring_finger_proximal_angle}")

                    target_list = [
                        clip(int(index_finger_mcp_yaw_angle), -20, 20),
                        clip(int(index_finger_mcp_up_angle * 1.2), 0, 90),
                        clip(int(index_finger_proximal_angle), 0, 90),
                        clip(int(index_finger_distal_angle), 0, 90),

                        clip(int(middle_finger_mcp_yaw_angle) - 10, -20, 20),
                        clip(int(middle_finger_mcp_up_angle * 1.2), 0, 90),
                        clip(int(middle_finger_proximal_angle), 0, 90),
                        clip(int(middle_finger_distal_angle), 0, 90),

                        clip(int(ring_finger_mcp_yaw_angle) - 20, -20, 20),
                        clip(int(ring_finger_mcp_up_angle * 2.0), 0, 90),
                        clip(int(ring_finger_proximal_angle), 0, 90),
                        clip(int(ring_finger_distal_angle), 0, 90),
...
def main():
    rclpy.init()
    node = SimplePublisherNode("hand_ctrl_publisher")
    rclpy.spin(node)
    rclpy.shutdown()
